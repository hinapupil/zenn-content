...
:::message
この記事は加筆中です。現時点では「Craig の zip を手動でダウンロードしてコマンド実行」までの手順を公開しています。録音〜ダウンロードの完全自動化は「将来対応」セクションに記載しています。
:::
...
毎週 15〜20 分。これをコマンド 1 発にしたい。文字起こしの API は有料なので、ローカルの GPU で Whisper を動かす。
...
- **文字起こしは無料**: Whisper をローカル GPU で動かすので API 課金なし
- **Craig の話者分離活用**: diarization ライブラリ不要
- **テキスト参加者も自動取得**: Discord Bot API で漏れなし

NotebookLM などの SaaS でも文字起こし→要約はできるが、文字起こし API の課金を避けたい場合や、議事録フォーマットを細かくコントロールしたい場合はこのアプローチが向いている。

## 将来対応

現状はまだ「Craig の zip を手動でダウンロードしてコマンドを実行する」というステップが残っている。最終的には**会議が終わったら議事録の下書きが勝手にできている**状態を目指したい。
...
生成された議事録はあくまで下書き。人間がレビュー・修正してから Discord チャンネルに共有する流れは変わらない。進展があれば本記事を更新する。
